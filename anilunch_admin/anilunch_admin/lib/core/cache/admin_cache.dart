...
class AdminCache {
  ...
  Future<void> clear(String entityType) async {
    final db = _db;
    if (db == null) return;
    await db.replaceDataset(entityType, []);
  }

  Future<void> clearAll() async {
    final db = _db;
    if (db == null) return;
    for (final entity in ['orders', 'users', 'riders', 'vendors', 'analytics']) {
      await db.replaceDataset(entity, []);
    }
  }
  ...
  /// Falls back to a plain network fetch on a cold cache or when forceRefresh is true.
